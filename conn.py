...
class Conn:
    ...
    def get_all_properties(self):
        """
        Returns a list of properties with supervisor info.
        supervisor_name will be None if unassigned.
        """
        query = """
            SELECT
                p.id,
...
class Conn:
    ...
    def reassign_ticket_admin(self, ticket_id, new_admin_id, old_admin_id, changed_by_admin, reason, is_super_admin=False):
        ...
        try:
            with self.engine.begin() as conn:
                ...
                # if no rows exist yet, scalar() may return None
    ...
    def update_property(self, property_id, name, supervisor_id):
        ...
        with self.engine.begin() as conn:
            # If supervisor is None, allow unassigned
            ...
            # Otherwise validate supervisor is a Property Manager
            check_query = text("""
                SELECT id FROM admin_users
                WHERE id = :id AND admin_type = 'Property Manager'
            """)
    ...
    def insert_ticket_and_get_id(self, user_id, description, category, property_id, assigned_admin):
        ...
        Uses Kenya time instead of NOW().
        """
        insert_query = text("""
            INSERT INTO tickets 
            (user_id, issue_description, status, created_at, category, property_id, assigned_admin)
